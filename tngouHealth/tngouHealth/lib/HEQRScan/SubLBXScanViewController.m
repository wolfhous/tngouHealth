...
- (void)drawBottomItems
{
    ...
    _btnFlash.frame = CGRectMake(CGRectGetMaxX(_bottomItemsView.frame)/2+50, 0, size.width, size.height);
     [_btnFlash setImage:[UIImage imageNamed:@"qrcode_scan_btn_flash_nor"] forState:UIControlStateNormal];
    [_btnFlash addTarget:self action:@selector(openOrCloseFlash) forControlEvents:UIControlEventTouchUpInside];
    
    self.btnPhoto = [[UIButton alloc]init];
    _btnPhoto.frame = CGRectMake(_bottomItemsView.frame.size.width/2-115, 0, size.width, size.height);
